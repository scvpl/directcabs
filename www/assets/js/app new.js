...
function onDeviceReady()
{
  ...
  window.FirebasePlugin.onNotificationOpen(function(notification) {
      ...
      //console.log(myApp.views.current);
      console.log(myApp.views.current.router.url);
      if(notification.tap == true) {
        if(myApp.views.current.router.url != '/cabs/ride/') {
          //myApp.dialog.alert(title, notification.page, function () {
            //mainView.router.navigate("/cabs/ride/");
            //mainView.router.navigate("/");
          //});
        }
      } else {
        if(myApp.views.current.router.url != '/cabs/ride/') {
          //myApp.dialog.alert(title, notification.message, function () {
            //mainView.router.navigate("/cabs/ride/");
            //mainView.router.navigate("/");
          //});
        }
...
function initializeOLD(lat, long) {
  ...
  if(usermobile) {
        ...
        if(tripsnap.exists()) {
          ...
          if(tripStatus == 'started' || tripStatus == 'accepted') {
            ...
            firebase.database().ref('sr_driver/'+userNum+'/profile').once('value').then(function(drsnap){
              initialize(drsnap.val().latitude, drsnap.val().longitude);
            ...
            firebase.database().ref('direct_trips/'+tripid).once('value').then(function(dtripsnap){
              ...
              //calcRoute(pickLatLong, dropLatLong);
    ...
    setTimeout(function(){
      ...
      // $$(".submitRide").on("click", function (e) {
        ...
        '<div class="item-media"><img src="img/route.png" width="40" style="border-radius:50%;"></div>'+
  ...
  $$(".phonesubmit").on("click", function (e) {
    ...
    if(verifymobile && (verifymobile.length >= 10) && (reminder == 0 || reminder == 1)) {
        ...
        if(myApp.device.ios) {
          ...
          window.FirebasePlugin.verifyPhoneNumber(verifymobile, 0, function(verification) {
              // pass verificationId to signInWithVerificationId
            ...
            cordova.plugins.locationServices.geolocation.getCurrentPosition(function(locationCurr) {
              ...
              directTripRef.once("value").then(function(directtrip){
                firebase.database().ref("direct_trips/"+drKey+"/drivers/"+userNum).once("value").then(function(tripsnap){
                  ...
                  console.log("dToPick - "+dToPick);
                  var price = tripsnap.val().km_price;
                  console.log("driverPrice - "+price);
                  $.ajax({
                    //url: 'https://maps.googleapis.com/maps/api/distancematrix/json?origins='+pickLocArray[0]+','+pickLocArray[1]+'&destinations='+latLong+'&alternatives=true&mode=driving&language=en-EN&sensor=false&key=AIzaSyC0yUYRKI9FcAaVrOziKoVYiJXGuNqmSv8',
                    url: 'https://www.socialrecharge.com/brand/firebase/sr_directcabs.php',
                    type: 'POST',
                    ...
                    success: function (result)
                    {
                      ...
                      console.log("stopLatLong :", stopLatLong);
                      ...
                      console.log("distMeter :", distMeter);
                      if(distMeter < 100) {
                      ...
                      .once("value", function(tripdaysnap){
                        ...
                        firebase.database().ref("users_credit/"+riderNum+"/total").once("value").then(function(usersnap){
                          console.log("usersnap val :", usersnap.val());
            ...
            if(eleButton) {
              ...
              $.each(eleButton, function (j) {
                if(element[i].buttons[j].type == 'web_url') {
                  ...
                  var pType = "'" + element[i].buttons[j].type + "'";
                  var pTitle = "'" + element[i].buttons[j].title + "'";
                  var titlecolor = (element[i].buttons[j].title.toLowerCase() == 'stop') ? 'color-red' : 'color-blue';
...
{
    mainView.router.navigate("/earnings/");
}
function filterData()
{
    ...
    if(datefrom) {
        ...
        mainView.router.navigate("/earnings/?pDate="+unix+"&pEndDate="+pEndDate+"&refund="+refundVal);
    } else {
        var today = new Date();
        console.log(today);
        var unix2 = Math.round(today/1000);
